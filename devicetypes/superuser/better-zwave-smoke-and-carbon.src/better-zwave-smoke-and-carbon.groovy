...
def zwaveEvent(physicalgraph.zwave.commands.alarmv2.AlarmReport cmd, results) {
	if (cmd.zwaveAlarmType == physicalgraph.zwave.commands.alarmv2.AlarmReport.ZWAVE_ALARM_TYPE_SMOKE) {
		...
		case 13:  // sent every hour -- not sure what this means, just a wake up notification?
            ...
            log.debug "Battery: prevBattery == ${prevBattery.value}, ${prevBattery.date}, ${(new Date().time - prevBattery.date.time)/60000}"
			if (!prevBattery || (new Date().time - prevBattery.date.time)/60000 >= 60 * 12) {
            	log.debug "Fetch updated battery value"
...
private updateResponsiveness() {
	log.debug "updateResponsiveness: state.lastResponse == ${state.lastResponse}"
...
//	    log.debug "lastResponseAge == ${lastResponseAge}"
